...
module Rack
  ...
  class S3Rack
    def initialize app
      ...
      @s3_credentials = ModelDocument.new.document
    end
    def call env
      s3_response = if env["PATH_INFO"] =~ /^\/s3\/(\d*)\/(\d*)/
        ...
        response = unless model_document
          ...
          p "ESH: s3=#{s3.class.name}"
          s3_config = s3.instance_variable_get '@config'
          s3.instance_variable_set '@config', s3_config.with({:http_handler => AWS::Http::HTTPartyHandler.new(:timeout => 5)})
