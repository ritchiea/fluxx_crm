  ...
  def add_workflow
    if model_class && model_class.public_method_defined?(:current_allowed_events)
      ...
      self.post do |pair|
        ...
        send :instance_variable_set, "@edit_enabled", (action_enabled || current_user.is_admin? || model.admin_edit_allowed_for_user?(current_user))
        send :instance_variable_set, "@delete_enabled", (action_enabled || current_user.is_admin? || model.admin_edit_allowed_for_user?(current_user))
