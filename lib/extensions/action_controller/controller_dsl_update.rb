  ...
  define_method :perform_update_with_specific do |*params|
    ...
    if @check_for_workflow
      ...
      if event_action
        ...
        if controller
          if event_allowed
            if (model.is_non_validating_event?(event_action) || model.valid?) && model.insta_fire_event(event_action, fluxx_current_user) && model.fire_state_based_validations && model.errors.empty?
